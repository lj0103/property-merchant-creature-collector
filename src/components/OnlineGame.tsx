    ...
    <main className="board">
      ...
      <div className="layout">
        ...
        <section className="market">
          <div className="market-title"><div><p className="eyebrow">服务器同步 · 羁绊先抵扣，灵珠付余额</p><h2>雾岚精灵市集</h2></div><p>剩余牌量 · {[3,2,1].map((level)=>{const tier=level as 1|2|3;return `L${level} ${game.decks[tier].length+game.market[tier].length}/${CARD_COUNTS_BY_LEVEL[tier]}`;}).join(' / ')}</p></div>
        ...
        <aside className="right-panel panel player-mat">
          {activePlayer && <>
            ...
            <h3 className="section-label">预定精灵 <small className="section-help">羁绊已自动抵扣</small><span>{activePlayer.reservedCards.length}/3</span></h3>

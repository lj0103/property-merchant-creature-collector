  ...
  return (
    <article className={`creature element-${card.element} ${able ? 'affordable' : ''}`}>
      <div className="card-top">
        ...
        <span className="card-element-mark" title={`捕捉后获得${ENERGY_LABELS[card.element]}系永久羁绊`}><GemIcon type={card.element} size="summary"/></span>
      ...
      <div className="cost-label">羁绊抵扣后所需灵珠</div>
      <div className="costs">
      ...
      <div className="card-actions">
        <button className="capture" disabled={!able} onClick={() => confirm(`是否捕捉「${card.name}」？`) && capture(card.id, source)}>{able ? '缔结契约' : '折后灵珠不足'}</button>
